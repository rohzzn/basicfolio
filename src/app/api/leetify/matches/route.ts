...
export async function GET(req: Request) {
  ...
  if (!leetifyId && !steam64Id) {
    return NextResponse.json({ error: "Missing steam64_id or id query param" }, { status: 400 });
  }

  try {
    const upstreamUrl = new URL("https://api-public.cs-prod.leetify.com/v3/profile/matches");
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          error: "Failed to fetch Leetify matches",
    ...
    console.error("Error fetching Leetify matches:", error);
    return NextResponse.json({ error: "Failed to fetch Leetify matches" }, { status: 500 });
  }
}
