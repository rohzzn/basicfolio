...
const PLACE_STYLE: Record<number, string> = {
  ...
  2: 'bg-zinc-100 dark:bg-neutral-700/50 text-zinc-600 dark:text-neutral-300 border-zinc-200 dark:border-neutral-600',
...
const GAME_COLORS: Record<string, string> = {
  ...
  'PUBG':    'text-green-600 dark:text-neutral-200',
};
...
export default function TournamentTimeline() {
  ...
  return (
    <div className="my-8 not-prose">
      <p className="text-xs uppercase tracking-wider text-zinc-400 dark:text-neutral-400 font-medium mb-3">Tournament History</p>

      <div className="flex gap-3 mb-4 flex-wrap">
        {['all', ...ALL_GAMES].map(g => (
          <button key={g} onClick={() => setFilter(g)}
            className={`text-xs px-2.5 py-1 rounded-md border transition-colors ${filter === g ? 'bg-zinc-900 dark:bg-neutral-100 text-white dark:text-neutral-900 border-zinc-900 dark:border-neutral-100' : 'border-zinc-200 dark:border-neutral-700 text-zinc-500 dark:text-neutral-400 hover:border-zinc-400 dark:hover:border-neutral-500'}`}>
            {g === 'all' ? 'All games' : g}
          </button>
        ))}
      </div>
      ...
      <div className="relative">
        <div className="absolute left-[52px] top-0 bottom-0 w-px bg-zinc-100 dark:bg-neutral-800" />
        <div className="space-y-3">
          {visible.map((ev, i) => (
            <div key={i} className="flex items-start gap-4 relative">
              <div className="w-10 text-right flex-shrink-0 pt-2.5">
                <span className="text-xs text-zinc-400 dark:text-neutral-400">{ev.year}</span>
              </div>
              <div className="w-3 h-3 rounded-full bg-zinc-200 dark:bg-neutral-700 flex-shrink-0 mt-2.5 z-10 relative" />
              <div className={`flex-1 flex items-center justify-between px-3 py-2.5 rounded-md border ${PLACE_STYLE[ev.place]}`}>
                <div>
                  ...
                  <p className="text-xs text-zinc-600 dark:text-neutral-400 mt-0.5">{ev.event}</p>
                </div>
      ...
      <div className="mt-4 flex gap-3 text-xs text-zinc-400 dark:text-neutral-400">
        <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-amber-400" />1st place</div>
        <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-zinc-300 dark:bg-neutral-500" />2nd place</div>
