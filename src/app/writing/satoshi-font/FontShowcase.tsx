...
export default function FontShowcase() {
  ...
  return (
    <div className="my-8 not-prose">
      <p className="text-xs uppercase tracking-wider text-zinc-400 dark:text-zinc-400 font-medium mb-3">Satoshi Font Demo</p>
          ...
          <div className="p-4">
            <div className="flex items-center gap-4 mb-5 flex-wrap">
              <div className="flex-1 min-w-40">
                <p className="text-xs text-zinc-400 dark:text-zinc-400 mb-1.5">Preview text</p>
              ...
              <div>
                <p className="text-xs text-zinc-400 dark:text-zinc-400 mb-1.5">Size: {size}px</p>
              ...
              {WEIGHTS.map(w => (
                <div key={w.value}
                  ...
                  <div className="flex items-baseline justify-between mb-1">
                    <span className="text-[10px] text-zinc-400 dark:text-zinc-400 uppercase tracking-wider">{w.label} — {w.value}</span>
                  </div>
            ...
            <div className="space-y-4">
              {COMPARE_FONTS.map(f => (
                <div key={f.label} className={`px-3 py-2.5 rounded-md border ${f.label === 'Satoshi' ? 'border-zinc-300 dark:border-zinc-600 bg-zinc-50 dark:bg-zinc-800/50' : 'border-zinc-100 dark:border-zinc-800'}`}>
                  <span className={`text-[10px] uppercase tracking-wider mb-1 block ${f.label === 'Satoshi' ? 'text-zinc-600 dark:text-zinc-300 font-medium' : 'text-zinc-400 dark:text-zinc-400'}`}>{f.label}</span>
          ...
          <div className="p-4 space-y-4" style={{ fontFamily: 'Satoshi, sans-serif' }}>
            {SAMPLE_STRINGS.map((s, i) => (
              <div key={i} className="border-b border-zinc-100 dark:border-zinc-800 last:border-0 pb-3 last:pb-0">
                ...
                <p className="text-[10px] text-zinc-400 dark:text-zinc-400 mt-0.5">{['Black 900', 'Bold 700', 'Medium 500', 'Regular 400', 'Light 300'][i]} — {Math.max(12, 36 - i * 5)}px</p>
              </div>
            ))}
          </div>
        )}
