    ...
    <article className="max-w-3xl pb-8 pt-0 px-4 sm:px-0">
      <header className="mb-8">
        <div className="flex items-start justify-between gap-4 mb-4">
          <h1 className="text-lg font-medium dark:text-paper">How to Build Discord Profile Widgets</h1>
      ...
      <div className="text-sm max-w-3xl">
        ...
        <h2 className="text-base font-medium mt-8 mb-3 dark:text-paper">
          <span className="text-zinc-400 dark:text-neutral-400 font-normal mr-2">01</span>Setting Up Your Application
        </h2>
        ...
        <div className="bg-zinc-100 dark:bg-neutral-800 p-4 rounded-lg mb-6">
...
findByProps("getAll").getAll().find(e=>e.getName() === "ApexExperimentStore")
  .createOverride("2026-03-widget-config-editor", 1)`}
        ...
        <h2 className="text-base font-medium mt-8 mb-3 dark:text-paper">
          <span className="text-zinc-400 dark:text-neutral-400 font-normal mr-2">02</span>Building the Widget
        </h2>
        <p className="text-zinc-600 dark:text-neutral-400 mb-4 text-sm">
        ...
        <div className="rounded-lg overflow-hidden mb-6 border border-zinc-200 dark:border-neutral-700">
          <Image
        ...
        <h2 className="text-base font-medium mt-8 mb-3 dark:text-paper">
          <span className="text-zinc-400 dark:text-neutral-400 font-normal mr-2">03</span>OAuth Setup
        </h2>
        <p className="text-zinc-600 dark:text-neutral-400 mb-4 text-sm">
        ...
        <p className="text-zinc-600 dark:text-neutral-400 mb-6 text-sm">
          Scroll down to the OAuth2 URL Generator. Check the scopes <code className="font-mono text-xs bg-zinc-100 dark:bg-neutral-800 px-1 py-0.5 rounded">openid</code> and <code className="font-mono text-xs bg-zinc-100 dark:bg-neutral-800 px-1 py-0.5 rounded">sdk.social_layer</code>, select your redirect URI, and copy the generated URL. Then change <code className="font-mono text-xs bg-zinc-100 dark:bg-neutral-800 px-1 py-0.5 rounded">response_type=code</code> to <code className="font-mono text-xs bg-zinc-100 dark:bg-neutral-800 px-1 py-0.5 rounded">response_type=token</code> in the URL and open it once to confirm there are no scope errors. If you get an invalid scopes error, the Social SDK form from step one was likely not submitted.
        </p>

        <h2 className="text-base font-medium mt-8 mb-3 dark:text-paper">
          <span className="text-zinc-400 dark:text-neutral-400 font-normal mr-2">04</span>Application Identities (Personal Use Only)
        </h2>
        <p className="text-zinc-600 dark:text-neutral-400 mb-4 text-sm">
        ...
        <p className="text-zinc-600 dark:text-neutral-400 mb-4 text-sm">
          Then go to the Bot page in the Developer Portal and reset the token. Copy it. Take the sample JSON from the widget editor, add a <code className="font-mono text-xs bg-zinc-100 dark:bg-neutral-800 px-1 py-0.5 rounded">username</code> field at the root with any value, then stringify it in the browser console:
        </p>
...
{`Invoke-RestMethod \`
  ...
  -Body '{jsonString}'`}
        ...
        <h2 className="text-base font-medium mt-8 mb-3 dark:text-paper">
          <span className="text-zinc-400 dark:text-neutral-400 font-normal mr-2">05</span>Widget Management With a Bot
        </h2>
        <p className="text-zinc-600 dark:text-neutral-400 mb-4 text-sm">
        ...
        <p className="text-zinc-600 dark:text-neutral-400 mb-4 text-sm">
          The setup command walks the user through authorizing your application with the <code className="font-mono text-xs bg-zinc-100 dark:bg-neutral-800 px-1 py-0.5 rounded">sdk.social_layer</code> scope and then verifying their account with whatever external service your widget represents. The refresh command is what actually sends data to Discord. The endpoint for that is:
        </p>
...
{`PATCH https://discord.com/api/v9/applications/{appId}/users/{userId}/identities/0/profile`}
        ...
        <p className="text-zinc-600 dark:text-neutral-400 mb-4 text-sm">
          The identity ID (the <code className="font-mono text-xs bg-zinc-100 dark:bg-neutral-800 px-1 py-0.5 rounded">0</code> at the end of that URL) can technically be any unique value but setting it to 0 works consistently. The request body structure mirrors the JSON from the Sample Data tab with the real values substituted in for each user. The three field types are: type 1 for strings, type 2 for numbers, type 3 for images with a URL. For example:
        </p>
  ...
  "data": {
    "dynamic": [
      ...
      { "type": 3, "name": "avatar",       "value": { "url": "https://..." } }
        ...
        <h2 className="text-base font-medium mt-8 mb-3 dark:text-paper">
          <span className="text-zinc-400 dark:text-neutral-400 font-normal mr-2">06</span>Adding the Widget to Your Profile
        </h2>
        <p className="text-zinc-600 dark:text-neutral-400 mb-4 text-sm">
          Adding a widget to your profile currently has to be done through the API directly as Discord has not exposed it in the normal UI yet. The Discord Previews community server has a thread with ready-to-run browser snippets that handle this for you, covering both directly pinning the widget to your profile and adding it to the Add Widget menu at the top right of your profile page. You will also want to make sure the experiment <code className="font-mono text-xs bg-zinc-100 dark:bg-neutral-800 px-1 py-0.5 rounded">2026-03-application-widget-v2-renderer</code> is set to Variant 1 on your account.
        </p>
        ...
        <div className="rounded-lg overflow-hidden mb-8 border border-zinc-200 dark:border-neutral-700 max-w-sm mx-auto">
          <Image
