...
export default function BuildCosts() {
  ...
  return (
    <div className="my-8 not-prose">
      <p className="text-xs uppercase tracking-wider text-zinc-400 dark:text-neutral-400 font-medium mb-3">Build Cost Reference</p>
      <div className="border border-zinc-200 dark:border-neutral-800 rounded-lg p-4">
        <div className="mb-5">
          <p className="text-xs text-zinc-500 dark:text-neutral-400 mb-2">Set your hand</p>
            ...
            {RES_ORDER.map(r => (
              <div key={r} className="flex items-center gap-1.5">
                ...
                <div className="flex flex-col gap-px">
                  <button onClick={() => adj(r, 1)} className="text-[10px] leading-none text-zinc-400 hover:text-zinc-700 dark:hover:text-neutral-200 transition-colors">▲</button>
                  <button onClick={() => adj(r, -1)} className="text-[10px] leading-none text-zinc-400 hover:text-zinc-700 dark:hover:text-neutral-200 transition-colors">▼</button>
                </div>
                <span className="text-xs text-zinc-500 dark:text-neutral-400 capitalize">{r}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-2">
          {BUILDINGS.map(b => {
            ...
            return (
              <div key={b.name}
                className={`flex items-center justify-between px-3 py-2.5 rounded-md border transition-all duration-200 ${ok ? 'border-emerald-300 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-900/20' : 'border-zinc-100 dark:border-neutral-800 bg-zinc-50/50 dark:bg-neutral-800/30'}`}
              >
                <div className="flex items-center gap-3">
                  <span className={`text-xs font-medium ${ok ? 'text-emerald-700 dark:text-emerald-400' : 'text-zinc-600 dark:text-neutral-400'}`}>
                    {b.name}
                  </span>
                  {ok && <span className="text-[10px] text-emerald-600 dark:text-emerald-500">can build</span>}
                </div>
                <div className="flex items-center gap-1">
                  <span className="text-xs text-zinc-400 dark:text-neutral-400 mr-1">{b.vp} VP</span>
        ...
        <p className="text-[10px] text-zinc-400 dark:text-neutral-400 mt-3">Colour key: green=wood, orange=brick, lime=sheep, yellow=wheat, slate=ore</p>
      </div>
    </div>
  );
}
